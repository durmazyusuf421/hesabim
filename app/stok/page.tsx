...
export default function StokKartlari() {
  ...
  useEffect(() => {
    ...
    // GÜVENLİK KİLİDİ GÜNCELLENDİ: Hem Toptancı Hem Market girebilir
  ...
  return (
    <div className="bg-slate-100 font-sans h-screen flex overflow-hidden text-slate-800">
      ...
      {/* DİNAMİK SOL MENÜ */}
      <aside className="w-56 bg-slate-900 text-slate-300 flex flex-col shrink-0 text-sm border-r border-slate-800 print:hidden z-20">
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
          {isYonetici || aktifSirket.rol === "PERAKENDE" ? (
        ...
        {!hasAccess ? (
            ...
            <>
                ...
                <div className="h-10 bg-slate-200 border-b border-slate-300 flex items-center px-4 shrink-0 space-x-4 print:hidden">
                    <span className="text-xs font-bold text-slate-600 uppercase">Stok Yönetimi</span>
      ...
      {modalAcik && hasAccess && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50">
          <div className="bg-slate-100 rounded shadow-2xl w-full max-w-xl overflow-hidden border border-slate-400">
            <div className={`border-b border-slate-300 p-3 flex justify-between items-center ${aktifSirket.rol === 'TOPTANCI' ? 'bg-slate-200' : 'bg-cyan-50'}`}>
