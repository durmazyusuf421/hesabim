...
export default function PortalTahsilatSayfasi() {
    ...
    async function verileriGetir(sirketId: number) {
        ...
        try {
            ...
            setCariler(firmalar.sort((a,b) => a.isim.localeCompare(b.isim)));
    ...
    const evrakKaydet = async () => {
        ...
        try {
            ...
            insertData.firma_id = seciliGercekCari.gercekId;
            ...
            // Güncel bakiyeyi veritabanından çek (stale state sorununu önler)
            const { data: guncelFirma } = await supabase.from("firmalar").select("bakiye").eq("id", seciliGercekCari.gercekId).single();
            const guncelBakiye = parseTutarToFloat(guncelFirma?.bakiye);
            const bakiyeDegisimi = evrak.islemTipi === "Tahsilat" ? -toplamTutarFloat : toplamTutarFloat;
            const yeniBakiye = guncelBakiye + bakiyeDegisimi;
            ...
            // Kayıt sonrası: cari seçili kalsın, bakiye güncellensin
            const kaydedilenCariId = evrak.cariId;
            const kaydedilenCariAdi = evrak.cariAdi;
            setEvrak(prev => ({
                ...prev,
                seri: prev.islemTipi === "Tahsilat" ? "THS" : "TDY",
                sira: Math.floor(Math.random() * 10000).toString(),
                bakiye: yeniBakiye,
                proje: "", personel: "", aciklama: ""
            }));
            setKalemler([{ id: Date.now(), cinsi: "Nakit", adi: "KASA", tutar: "", aciklama: "" }]);
    ...
    const getCariIsmiGecmis = (h: CariHareketRow) => {
        if (h.firma_id) { const f = cariler.find(c => c.gercekId === h.firma_id); if (f) return f.isim; }
                    ...
                    <div className="p-3 space-y-3">
                        ...
                        <div>
                            ...
                            <select value={evrak.cariId} onChange={handleCariSecim} className={`input-kurumsal w-full font-semibold text-sm py-3 cursor-pointer ${!evrak.cariId ? "bg-yellow-50 border-yellow-300" : "bg-amber-50"}`}>
                                ...
                                {cariler.map(c => <option key={c.id} value={c.id}>{c.isim} (#{c.gercekId})</option>)}
                            </select>
                        </div>
