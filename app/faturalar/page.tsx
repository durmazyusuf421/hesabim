                                    ...
                                    <>
                                        <div className="grid grid-cols-2 gap-2">
                                            <div style={{ userSelect: 'none', WebkitUserSelect: 'none' } as React.CSSProperties}>
                                                ...
                                                <input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" value={item.miktar} onFocus={(e) => e.target.select()} onChange={(e) => { const val = e.target.value.replace(',', '.'); if (/^\d*\.?\d*$/.test(val) || val === '') satirGuncelle(index, "miktar", val); }} autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck={false} style={{ userSelect: 'none', WebkitUserSelect: 'none' } as React.CSSProperties} className="input-kurumsal w-full text-center text-[12px] font-bold" />
                                            </div>
                                            <div>
