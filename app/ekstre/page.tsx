        ...
        {!hasAccess ? (
            ...
            <>
                ...
                <div className="h-12 bg-slate-200 border-b border-slate-300 flex items-center px-4 shrink-0 space-x-4 print:hidden">
                    <span className="text-xs font-bold text-slate-600 uppercase">Cari Seçimi</span>
                    <select 
                        value={seciliFirmaId} 
                        onChange={(e) => setSeciliFirmaId(e.target.value)}
                        className="flex-1 max-w-lg text-sm px-3 py-1.5 border border-slate-300 rounded shadow-inner outline-none focus:border-blue-500 font-bold text-slate-700"
                    >
                ...
                <div className="h-8 bg-slate-200 border-t border-slate-300 flex items-center justify-between px-4 text-[10px] text-slate-600 font-bold shrink-0 print:hidden">
                    ...
                    <span className="text-blue-700">Müşteri Bakiyesi: {Math.abs(yuruyenBakiye).toLocaleString('tr-TR')} TL {yuruyenBakiye > 0 ? 'Borçlu' : yuruyenBakiye < 0 ? 'Alacaklı' : 'Kapalı'}</span>
                </div>
            </>
        )}
      </main>
