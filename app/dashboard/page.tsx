...
export default function AnaSayfa() {
  ...
  async function verileriTopla(sirketId: number) {
      setYukleniyor(true);
      try {
          // PC TASARIMINI BOZMADAN EKLENEN HIZLANDIRICI (Promise.all)
  ...
  return (
    <div className="bg-[#f0f4f8] font-sans h-[100dvh] flex overflow-hidden text-slate-800 w-full max-w-[100vw]">
      ...
      {/* MOBİL KARARTMA EKRANI (PC'de görünmez) */}
      {mobilMenuAcik && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-40 md:hidden" onClick={() => setMobilMenuAcik(false)}></div>
      )}

      {/* SOL MENÜ - PC'de sabit, Mobilde yandan açılır */}
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 md:w-56 bg-slate-900 text-slate-300 flex flex-col shrink-0 text-sm border-r border-slate-800 shadow-2xl md:shadow-none transform transition-transform duration-300 ease-in-out ${mobilMenuAcik ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="h-16 md:h-20 flex items-center justify-between md:justify-center border-b border-slate-700 bg-slate-950 px-4 md:px-2 text-center relative">
            <div className="flex flex-col items-center w-full">
                <span className={`text-[10px] uppercase mb-0.5 ${sirketRol === 'TOPTANCI' ? 'text-orange-500' : 'text-cyan-500'}`}>
                    {isYonetici ? 'SİSTEM YÖNETİCİSİ' : 'PERSONEL HESABI'}
                </span>
                <span className="text-xs truncate w-full">{aktifSirket?.email || aktifSirket?.isletme_adi || "Yükleniyor..."}</span>
            </div>
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
          {isYonetici ? (
              <Link href="/ayarlar" onClick={handleMenuClick} className={`flex items-center px-2 py-2 transition w-full text-xs uppercase tracking-widest rounded ${pathname === "/ayarlar" ? "bg-slate-800 text-white" : "text-slate-300 hover:text-white"}`}><i className="fas fa-cog w-6"></i> Ayarlar</Link>
          ) : null}
      ...
      <main className="flex-1 flex flex-col h-[100dvh] overflow-hidden relative w-full">
        ...
        {/* ÜST HEADER - PC'de aynı, Mobilde Hamburger var */}
        ...
        {yukleniyor || !aktifSirket ? (
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 animate-in fade-in duration-300 w-full overflow-x-hidden">
                <div className="max-w-[1600px] mx-auto space-y-4 md:space-y-6">
                    ...
                    {/* TIKLANABİLİR KPI KARTLARI (PC tasarımına dokunulmadı, Mobilde 2 sütun) */}
                        ...
                        {[
                            ...
                            <Link href={kpi.link} key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5 flex flex-col hover:shadow-md transition-all duration-300 group hover:border-blue-300 cursor-pointer">
                                <div className="flex justify-between items-center mb-2 md:mb-4">
                                    ...
                                    <div className={`w-7 h-7 md:w-8 md:h-8 rounded-lg ${kpi.bg} ${kpi.color} flex items-center justify-center group-hover:scale-110 transition-transform shrink-0`}>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                        ...
                        {/* AYLIK GELİRLER GRAFİĞİ (PC TASARIMI KORUNDU) */}
                    ...
                    {/* ALT BÖLÜM: SİPARİŞLER (PC'de Tablo, Mobilde Kart) */}
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 w-full">
                        ...
                        <div className="flex flex-col gap-4 md:gap-6 w-full">
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5 flex flex-col gap-4">
                                <div><div className="flex justify-between items-end mb-2"><span className="text-xs md:text-sm font-bold text-slate-700">Tahsilat Oranı</span><span className="text-[10px] md:text-xs font-black text-blue-600">% 85</span></div><div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-blue-500 h-2 rounded-full" style={{width: '85%'}}></div></div></div>
                                <div><div className="flex justify-between items-end mb-2"><span className="text-xs md:text-sm font-bold text-slate-700">Depo Doluluk</span><span className="text-[10px] md:text-xs font-black text-orange-500">{toplamUrun} Ürün</span></div><div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-orange-400 h-2 rounded-full" style={{width: '60%'}}></div></div></div>
                            </div>
                        </div>
                    </div>
                </div>
